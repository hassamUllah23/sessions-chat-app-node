              ...
              if (result) {
                ...
                newSession.save().then(async (newSavedSession) => {
                  if (newSavedSession) {
                    ...
                    // Run a command
                    const command = `pm2 start "yarn ${process.env.NODE_ENV}:cs ${populatedSession?._id.toString()}" --name "scp-${populatedSession?._id.toString()}"`;
                    console.log({ command });
                    ...
                    //       sessionId
                    //     },
                    //     script: 'app.js', // Path to your application script
                    //     name: 'my-app',   // Name of the process
                    //     exec_mode: 'cluster', // Execution mode
                    //     instances: 2,     // Number of instances to run
                    //     max_memory_restart: '100M', // Maximum memory limit
